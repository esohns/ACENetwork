...
#include "test_i_listener_common.h"
  ...
  while ((option = argumentParser ()) != EOF)
  {
    switch (option)
    {
      ...
      case 'h':
      {
        ...
        serverAddress_out =
          Net_Common_Tools::stringToIPAddress (address_string, FTP_DEFAULT_SERVER_CONTROL_PORT);
        break;
      }
      case 'l':
      {
         ...
         Test_I_SignalHandler& signalHandler_in)
{
  ...
  FTP_Client_MessageAllocator_t message_allocator (TEST_I_MAX_MESSAGES, // maximum #buffers
  ...
  struct FTP_Client_ModuleHandlerConfiguration modulehandler_configuration_2;
  struct FTP_Client_StreamConfiguration stream_configuration;
  struct FTP_Client_StreamConfiguration stream_configuration_2;
...
//  struct FTP_Client_StreamConfiguration stream_configuration;
  ...
  //    Test_I_LISTENER_SINGLETON::instance ();
  //else
  //  configuration_in.signalHandlerConfiguration.listener =
  //    Test_I_ASYNCHLISTENER_SINGLETON::instance ();
...
//#endif // SSL_SUPPORT
  ...
  if (!configuration_in.listener->initialize (configuration_in.connectionConfiguration_2))
  {
    ACE_DEBUG ((LM_ERROR,
  ...
  if (UIDefinitionFileName_in.empty ())
  {
    ...
    if (unlikely (handle == ACE_INVALID_HANDLE))
    {
      ACE_DEBUG ((LM_ERROR,
                  ...
                  ACE_TEXT (Net_Common_Tools::IPAddressToString (configuration_in.connectionConfiguration.socketConfiguration.address).c_str ())));
    ...
    // *TODO*: login and initiate file retrieval
    ACE_ASSERT (false);
  } // end IF
           ...
           ACE_TCHAR* argv_in[])
{
  ...
  // step1a set defaults
  //bool request_broadcast_replies = FTP_Client_DEFAULT_FLAGS_BROADCAST;
  ...
  //    Test_I_DEFAULT_FTP_Client_SEND_REQUEST_ON_OFFER;
  bool use_reactor =
      (COMMON_EVENT_DEFAULT_DISPATCH == COMMON_EVENT_DISPATCH_REACTOR);
  ...
  struct FTP_Client_Configuration configuration_s;
