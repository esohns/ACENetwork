...
connection_setup_function (void* arg_in)
{
  ...
  BitTorrent_Client_ConnectionThreadData* data_p =
    static_cast<BitTorrent_Client_ConnectionThreadData*> (arg_in);
...
//    ACE_Guard<ACE_SYNCH_MUTEX> aGuard (data_p->CBData->GTKState.lock);
    ...
    Common_UI_GTKBuildersIterator_t iterator =
      data_p->CBData->GTKState.builders.find (ACE_TEXT_ALWAYS_CHAR (COMMON_UI_GTK_DEFINITION_DESCRIPTOR_MAIN));
    // sanity check(s)
    ACE_ASSERT (iterator != data_p->CBData->GTKState.builders.end ());
...
//  gdk_threads_enter ();
  ACE_NEW_NORETURN (connection_p,
                    bittorrent_Client_GUI_Connection (&data_p->CBData->GTKState,
...
//  gdk_threads_leave ();
  ...
  bittorrent_Client_Connection_Manager_t::CONNECTION_T* connection_2 = NULL;
  ...
  bittorrent_Client_Connector_t connector (connection_manager_p,
                                    data_p->configuration->streamConfiguration.statisticReportingInterval);
  bittorrent_Client_AsynchConnector_t asynch_connector (connection_manager_p,
                                                 data_p->configuration->streamConfiguration.statisticReportingInterval);
  bittorrent_Client_IConnector_t* connector_p = &connector;
  ...
  for (bittorrent_Client_PortRangesIterator_t iterator = data_p->phonebookEntry.ports.begin ();
...
connection_failed:
    ...
    if (!string_p)
      ACE_DEBUG ((LM_ERROR,
                  ACE_TEXT ("failed to Common_UI_Tools::Locale2UTF8(\"%s\"): \"%m\", continuing\n"),
                  ACE_TEXT (converter.str ().c_str ())));
    gdk_threads_enter ();
  ...
  if (!data_p->configuration->useReactor)
  {
    ACE_Time_Value deadline = COMMON_TIME_NOW +
                              ACE_Time_Value (bittorrent_CLIENT_CONNECTION_ASYNCH_TIMEOUT, 0);
    ACE_Time_Value delay (bittorrent_CLIENT_CONNECTION_ASYNCH_TIMEOUT_INTERVAL, 0);
    do
    {
  ...
  socket_connection_p =
      dynamic_cast<bittorrent_Client_ISocketConnection_t*> (connection_2);
  ...
  for (Stream_Iterator_t iterator_2 (*stream_p);
       ...
       iterator_2.advance ())
  {
    tail_p = const_cast<bittorrent_Client_Stream*> (stream_p)->tail ();
    if (current_p != tail_p)
      module_p = current_p;
  } // end FOR
  ACE_ASSERT (module_p);
  ...
  {
    ACE_DEBUG ((LM_ERROR,
                ACE_TEXT ("dynamic_cast<bittorrent_IControl> failed, returning\n")));
  ...
  connection_p->initialize (&const_cast<bittorrent_Client_SessionState&> (connection_2->state ()),
                            icontrol_p);
  { // synch access
#if defined (ACE_WIN32) || defined (ACE_WIN64)
    ACE_GUARD_RETURN (ACE_SYNCH_MUTEX, aGuard, data_p->CBData->GTKState.lock, result);
  ...
  try {
    ...
    ACE_DEBUG ((LM_ERROR,
                ACE_TEXT ("caught exception in bittorrent_IControl::registerc(), continuing\n")));
  }
  if (!result_2)
  {
    ACE_DEBUG ((LM_ERROR,
                ACE_TEXT ("failed to bittorrent_IControl::registerc(), returning\n")));
...
idle_add_channel_cb (gpointer userData_in)
{
  ...
  bittorrent_Client_GTK_HandlerCBData* data_p =
    static_cast<bittorrent_Client_GTK_HandlerCBData*> (userData_in);
...
//  ACE_ASSERT (data_p->eventSourceID); // *NOTE*: seems to be a race condition
  ...
  {
    ACE_DEBUG ((LM_ERROR,
                ACE_TEXT ("failed to Common_UI_Tools::Locale2UTF8(\"%s\"): \"%m\", aborting\n"),
                ACE_TEXT (data_p->label.c_str ())));
...
idle_finalize_UI_cb (gpointer userData_in)
{
  ...
  bittorrent_Client_GTK_CBData* data_p =
    static_cast<bittorrent_Client_GTK_CBData*> (userData_in);
...
idle_initialize_UI_cb (gpointer userData_in)
{
  ...
  for (bittorrent_Client_ServersIterator_t iterator_2 = data_p->phoneBook.servers.begin ();
       ...
       ++iterator_2)
  {
    ...
    if (network_map_iterator == network_map.end ())
    {
      ...
      std::string network_label =
        ((*iterator_2).second.netWork.empty () ? ACE_TEXT_ALWAYS_CHAR (bittorrent_CLIENT_PHONEBOOK_DEF_NETWORK_LABEL)
    ...
    // set active item
    if ((*iterator_2).first ==
        ACE_TEXT_ALWAYS_CHAR (bittorrent_CLIENT_DEFAULT_SERVER_HOSTNAME))
...
//  ACE_ASSERT (data_p->eventSourceID); // *NOTE*: seems to be a race condition
  ...
  bittorrent_Client_GUI_ConnectionsConstIterator_t iterator_3 =
  ...
  iterator_2 = data_p->GTKState->builders.find (data_p->timeStamp);
  // sanity check(s)
  if (iterator_2 == data_p->GTKState->builders.end ())
  {
    ACE_DEBUG ((LM_ERROR,
...
clean_up:
  ...
  {
    bittorrent_Client_GUI_Connections_t* connections_p = data_p->connections;
...
idle_update_display_cb (gpointer userData_in)
{
  ...
  bittorrent_Client_GUI_Connection* connection_p =
    bittorrent_Client_UI_Tools::current (data_p->GTKState,
                                  data_p->connections);
  if (!connection_p) // *NOTE*: most probable cause: no server page/corresponding connection (yet)
    return G_SOURCE_CONTINUE;

  // step1: retrieve active channel
  bittorrent_Client_GUI_MessageHandler* message_handler_p =
  ...
  try {
    ...
    ACE_DEBUG ((LM_ERROR,
                ACE_TEXT ("caught exception in bittorrent_Client_GUI_MessageHandler::update(), continuing\n")));
  }
...
//  ACE_ASSERT (data_p->eventSourceID); // *NOTE*: seems to be a race condition
  ...
  GtkToggleButton* toggle_button_p = NULL;
  Common_UI_GTKBuildersIterator_t iterator;
  bittorrent_Client_GUI_ConnectionsConstIterator_t iterator_2;
  const bittorrent_Client_SessionState* connection_state_p = NULL;
                           ...
                           gpointer userData_in)
{
  ...
  if (network_name_string == ACE_TEXT_ALWAYS_CHAR (bittorrent_CLIENT_PHONEBOOK_DEF_NETWORK_LABEL))
    network_name_string.clear ();
  bittorrent_Client_ServersIterator_t phonebook_iterator;
  ...
  bittorrent_LoginOptions login_options =
  ...
  gtk_entry_set_max_length (entry_p,
                            bittorrent_PRT_MAXIMUM_NICKNAME_LENGTH);
  ...
  do
  {
    ...
    // sanity check: <= bittorrent_CLIENT_CNF_bittorrent_MAX_NICK_LENGTH characters ?
    // *TODO*: support the NICKLEN=xxx "feature" of the server...
    if (login_options.nickName.size () > bittorrent_PRT_MAXIMUM_NICKNAME_LENGTH)
      login_options.nickName.resize (bittorrent_PRT_MAXIMUM_NICKNAME_LENGTH);
    ...
    { // synch access
      ...
      for (bittorrent_Client_GUI_ConnectionsConstIterator_t iterator_2 = data_p->connections.begin ();
  ...
  bittorrent_Client_ConnectionThreadData* connection_thread_data_p = NULL;
  ACE_NEW_NORETURN (connection_thread_data_p,
                    bittorrent_Client_ConnectionThreadData ());
                        ...
                        gpointer userData_in)
{
  ...
  {
    ACE_DEBUG ((LM_ERROR,
                ACE_TEXT ("failed to bittorrent_Client_GUI_Tools::current(), returning\n")));
    return;
  } // end IF
  ...
  {
    case bittorrent_CHARACTERENCODING_ASCII:
    {
      // *TODO*:
      ACE_ASSERT (false);
      break;
    }
    case bittorrent_CHARACTERENCODING_LOCALE:
    {
      message_string =
    ...
    case bittorrent_CHARACTERENCODING_UTF8:
    {
  ...
  {
    ACE_DEBUG ((LM_ERROR,
                ACE_TEXT ("failed to bittorrent_Client_GUI_Connection::getActiveHandler(), returning\n")));
    return;
  } // end IF
  const bittorrent_Client_GTK_HandlerCBData& cb_data_r = message_handler_p->get ();
  ...
  try {
    ...
    ACE_DEBUG ((LM_ERROR,
                ACE_TEXT ("caught exception in bittorrent_Client_IIRCControl::send(), continuing\n")));
  }
  ...
  try {
    ...
    ACE_DEBUG ((LM_ERROR,
                ACE_TEXT ("caught exception in bittorrent_Client_GUI_MessageHandler::queueForDisplay(), continuing\n")));
  }
                              ...
                              gpointer userData_in)
{
  ...
  try {
    data_p->controller->quit (ACE_TEXT_ALWAYS_CHAR (bittorrent_DEFAULT_LEAVE_REASON));
  } catch (...) {
    ACE_DEBUG ((LM_ERROR,
                ACE_TEXT ("caught exception in bittorrent_Client_IIRCControl::quit(), continuing\n")));
  }
  ...
  bittorrent_Client_GUI_ConnectionsConstIterator_t iterator =
                     ...
                     gpointer userData_in)
{
  ...
  // sanity check: <= bittorrent_CLIENT_CNF_bittorrent_MAX_NICK_LENGTH characters ?
  // *TODO*: support the NICKLEN=xxx "feature" of the server...
  if (nickname_string.size () > bittorrent_PRT_MAXIMUM_NICKNAME_LENGTH)
    nickname_string.resize (bittorrent_PRT_MAXIMUM_NICKNAME_LENGTH);

  try {
    data_p->controller->nick (nickname_string);
  } catch (...) {
    ACE_DEBUG ((LM_ERROR,
                ACE_TEXT ("caught exception in bittorrent_Client_IIRCControl::nick(), continuing\n")));
  }
...
//   user_string = g_value_get_string(&active_value);
  ...
  g_free (string_p);
  // sanity check(s): larger than bittorrent_CLIENT_CNF_bittorrent_MAX_NICK_LENGTH characters ?
  // *TODO*: support the NICKLEN=xxx "feature" of the server...
  if (username.size () > bittorrent_PRT_MAXIMUM_NICKNAME_LENGTH)
    username.resize (bittorrent_PRT_MAXIMUM_NICKNAME_LENGTH);
...
//   }
  ...
  try {
    ...
    ACE_DEBUG ((LM_ERROR,
                ACE_TEXT ("caught exception in bittorrent_Client_IIRCControl::who(), continuing\n")));
  }
}
                 ...
                 gpointer userData_in)
{
  ...
  // sanity check(s): larger than bittorrent_CLIENT_CNF_bittorrent_MAX_CHANNEL_LENGTH characters ?
  // *TODO*: support the CHANNELLEN=xxx "feature" of the server...
  if (channel_string.size () > bittorrent_PRT_MAXIMUM_CHANNEL_LENGTH)
    channel_string.resize (bittorrent_PRT_MAXIMUM_CHANNEL_LENGTH);
  ...
  try {
    ...
    ACE_DEBUG ((LM_ERROR,
                ACE_TEXT ("caught exception in bittorrent_Client_IIRCControl::join(), continuing\n")));
  }
                             ...
                             gpointer userData_in)
{
  ...
  try {
    ...
    ACE_DEBUG ((LM_ERROR,
                ACE_TEXT ("caught exception in bittorrent_Client_IIRCControl::list(), continuing\n")));
  }
}
                      ...
                      gpointer userData_in)
{
  ...
  bittorrent_UserMode mode = USERMODE_INVALID;
  ...
  try {
    data_p->controller->mode (connection_state_r.nickName,               // user mode
                              bittorrent_Tools::UserMode2Char (mode),           // corresponding mode char
                              !connection_state_r.userModes.test (mode), // enable ?
                              parameters);                               // parameters
  } catch (...) {
    ACE_DEBUG ((LM_ERROR,
                ACE_TEXT ("caught exception in bittorrent_Client_IIRCControl::mode(\"%s\"), continuing\n"),
                ACE_TEXT (bittorrent_Tools::UserMode2String (mode).c_str ())));
  }
}
                ...
                gpointer userData_in)
{
  ...
  //                             -1); // reset to default
    gtk_entry_set_text (entry_p,
                        ACE_TEXT_ALWAYS_CHAR (bittorrent_DEFAULT_AWAY_MESSAGE));
    gtk_editable_select_region (GTK_EDITABLE (entry_p),
                                0, -1);
  ...
  //                ACE_TEXT("away cancelled...\n")));
    ...
    if (away_message.empty ())
    {
      ...
      away_message = ACE_TEXT_ALWAYS_CHAR (bittorrent_DEFAULT_AWAY_MESSAGE);
    } // end IF
  } // end IF
  ...
  try {
    ...
    ACE_DEBUG ((LM_ERROR,
                ACE_TEXT ("caught exception in bittorrent_Client_IIRCControl::away(\"%s\"), continuing\n"),
                         ...
                         gpointer userData_in)
{
  ...
  bittorrent_ChannelMode mode           = CHANNELMODE_INVALID;
  ...
  else
  {
    ...
    if (result == 0)
    {
      ...
      need_parameter = true;
      entry_label =
        ACE_TEXT_ALWAYS_CHAR (bittorrent_CLIENT_GUI_DIALOG_ENTRY_TITLE_MODE_BAN);
    } // end IF
    else
    {
      result =
      ...
      else
      {
        ...
        if (result == 0)
        {
          ...
          need_parameter = true;
          entry_label =
            ACE_TEXT_ALWAYS_CHAR (bittorrent_CLIENT_GUI_DIALOG_ENTRY_TITLE_MODE_PASSWORD);
        } // end IF
        else
        {
          result =
          ...
          if (result == 0)
          {
            ...
            need_parameter = true;
            entry_label =
              ACE_TEXT_ALWAYS_CHAR (bittorrent_CLIENT_GUI_DIALOG_ENTRY_TITLE_MODE_USERLIMIT);
          }
          else
          {
            result =
                        ...
                        else
                        {
                          ...
                          else
                          {
                            ...
                            if (result == 0)
                            {
                              ...
                              need_parameter = true;
                              entry_label =
                                ACE_TEXT_ALWAYS_CHAR (bittorrent_CLIENT_GUI_DIALOG_ENTRY_TITLE_MODE_VOICE);
                            } // end IF
                            else
                            {
                              ACE_DEBUG ((LM_ERROR,
  ...
  // *NOTE*: see also: bittorrent_Client_MessageHandler ctor
  std::string page_tab_label_string;
  if (!bittorrent_Tools::isValidChannelName (data_p->id))
  {
    // --> private conversation window, modify label accordingly
...
//                  ACE_TEXT("channel mode cancelled...\n")));
  ...
  try {
    data_p->controller->mode (data_p->id,                         // channel name
                              bittorrent_Tools::ChannelMode2Char (mode), // corresponding mode char
                              !data_p->channelModes.test (mode),  // enable ?
                              parameters);                        // parameters
  } catch (...) {
    ACE_DEBUG ((LM_ERROR,
                ACE_TEXT ("caught exception in bittorrent_Client_IIRCControl::mode(\"%s\"), continuing\n"),
                ACE_TEXT (bittorrent_Tools::ChannelMode2String (mode).c_str ())));
  }
}
...
//                ACE_TEXT("channel topic cancelled...\n")));
  ...
  try {
    ...
    ACE_DEBUG ((LM_ERROR,
                ACE_TEXT ("caught exception in bittorrent_Client_IIRCControl::topic(), continuing\n")));
  }
                 ...
                 gpointer userData_in)
{
  ...
  { // --> inform the server
    ...
    try {
      ...
      ACE_DEBUG ((LM_ERROR,
                  ACE_TEXT ("caught exception in bittorrent_Client_IIRCControl::part(), continuing\n")));
    }
  } // end IF
                  ...
                  gpointer userData_in)
{
       ...
       iterator_2++)
  {
    try {
      ...
      ACE_DEBUG ((LM_ERROR,
                  ACE_TEXT ("caught exception in bittorrent_Client_IIRCControl::invite(), continuing\n")));
    }
  } // end FOR
}
                ...
                gpointer userData_in)
{
  ...
  try {
    ...
    ACE_DEBUG ((LM_ERROR,
                ACE_TEXT ("caught exception in bittorrent_Client_IIRCControl::userhost(), continuing\n")));
  }
}
       ...
       iterator++)
  {
    try {
      data_p->controller->kick (data_p->id,
                                *iterator,
                                ACE_TEXT_ALWAYS_CHAR (bittorrent_DEFAULT_KICK_REASON));
    } catch (...) {
      ACE_DEBUG ((LM_ERROR,
                  ACE_TEXT ("caught exception in bittorrent_Client_IIRCControl::kick(), continuing\n")));
    }
  } // end FOR
}
       ...
       iterator++)
  {
    ...
    try {
      data_p->controller->mode (data_p->id,
                                bittorrent_Tools::ChannelMode2Char (CHANNELMODE_BAN),
                                true,
                                parameters);
    } catch (...) {
      ACE_DEBUG ((LM_ERROR,
                  ACE_TEXT ("caught exception in bittorrent_IControl_T::mode(), continuing\n")));
    }
  } // end FOR
}
#ifdef __cplusplus
