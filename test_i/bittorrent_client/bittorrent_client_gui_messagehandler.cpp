...
BitTorrent_Client_GUI_MessageHandler::BitTorrent_Client_GUI_MessageHandler (struct BitTorrent_Client_GTK_CBData* CBData_in,
...
//    ACE_Guard<ACE_SYNCH_MUTEX> aGuard (CBData_->GTKState->lock);
  ...
  if (!BitTorrent_Tools::isValidChannelName (CBData_->id))
  {
...
//  ACE_Guard<ACE_SYNCH_MUTEX> aGuard (CBData_->GTKState->lock);
  ...
  {
    Common_UI_GTKBuildersIterator_t iterator =
        CBData_->builders.find (CBData_->builderLabel);
    if (iterator != CBData_->builders.end ())
    {
...
const BitTorrent_Client_GTK_CBData&
