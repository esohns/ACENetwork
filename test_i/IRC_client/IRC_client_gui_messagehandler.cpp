...
//  } // end lock scope
  ...
  { ACE_Guard<ACE_SYNCH_MUTEX> aGuard (CBData_.UIState->lock);
    CBData_.UIState->builders[CBData_.builderLabel] =
...
#if defined (GTK_USE)
    Common_UI_GTK_BuildersIterator_t iterator =
        CBData_.UIState->builders.find (CBData_.builderLabel);
    if (iterator != CBData_.UIState->builders.end ())
    {
...
#if defined (GTK_USE)
GtkWidget*
IRC_Client_GUI_MessageHandler::getTopLevelPageChild (bool lockedAccess_in) const
{
...
void
IRC_Client_GUI_MessageHandler::clearMembers (bool lockedAccess_in)
{
...
void
IRC_Client_GUI_MessageHandler::endMembers (bool lockedAccess_in)
{
