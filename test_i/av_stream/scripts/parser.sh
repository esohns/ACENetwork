...
SCRIPTS_DIRECTORY=${PROJECT_ROOT}/test_i/av_stream/scripts
...
TARGET_FILE=${TARGET_DIRECTORY}/test_i_av_stream_parser.h
SOURCE_FILE=${SCRIPTS_DIRECTORY}/test_i_av_stream_parser_patched.h
#[ ! -f ${SOURCE_FILE} ] && echo "ERROR: file ${SOURCE_FILE} not found, aborting" && exit 1
#cp -f ${SOURCE_FILE} ${TARGET_FILE}
#[ $? -ne 0 ] && echo "ERROR: failed to cp \"${SOURCE_FILE}\", aborting" && exit 1
#echo "copied \"$SOURCE_FILE\"..."
# clean up
SOURCE_FILE=test_i_av_stream_parser.h
#rm -f ${SOURCE_FILE}
#[ $? -ne 0 ] && echo "ERROR: failed to rm \"${SOURCE_FILE}\", aborting" && exit 1

#FILES="irc_parser.cpp irc_parser.h"
FILES="test_i_av_stream_parser.cpp"
for FILE in $FILES
do
