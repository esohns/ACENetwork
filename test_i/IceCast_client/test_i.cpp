...
do_print_usage (const std::string& programName_in)
{
  ...
  std::cout << ACE_TEXT_ALWAYS_CHAR ("-t        : trace information [")
            ...
            << ACE_TEXT_ALWAYS_CHAR ("]")
            << std::endl;
  std::cout << ACE_TEXT_ALWAYS_CHAR ("-u [URL]  : URL [")
            << ACE_TEXT_ALWAYS_CHAR (TEST_I_ICECAST_CLIENT_DEFAULT_URL)
            << ACE_TEXT_ALWAYS_CHAR ("]")
            << std::endl;
         ...
         Test_I_SignalHandler& signalHandler_in)
{
  ...
  allocator_configuration.defaultBufferSize =
    TEST_I_ICECAST_CLIENT_DEFAULT_BUFFER_SIZE;
           ...
           ACE_TCHAR* argv_in[])
{
  ...
  struct Test_I_IceCastClient_UI_CBData ui_cb_data;
...
//  lock_2 = &state_r.subscribersLock;
  ...
  url = ACE_TEXT_ALWAYS_CHAR (TEST_I_ICECAST_CLIENT_DEFAULT_URL);
