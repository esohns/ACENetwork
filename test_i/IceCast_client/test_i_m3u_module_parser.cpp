                                             ...
                                             bool& passMessageDownstream_out)
{
  ...
  const Test_I_IceCastClient_MessageData& data_r = data_container_r.getR ();
  ...
  if (!missing_bytes)
  {
    // check if the content IS a URL -> do not parse in that case
    iterator =
      data_r.headers.find (Common_String_Tools::tolower (ACE_TEXT_ALWAYS_CHAR (HTTP_PRT_HEADER_CONTENT_TYPE_STRING)));
    if (likely (iterator != data_r.headers.end () &&
                (*iterator).second == ACE_TEXT_ALWAYS_CHAR (HTTP_PRT_MIMETYPE_AUDIO_MPEGURL_STRING)))
    {
      ...
      { passMessageDownstream_out = false;
        ...
        inherited::headFragment_ = message_inout;

      std::string URL_string;
      ACE_Message_Block* message_block_p = inherited::headFragment_;
      do
      {
        URL_string.append (message_block_p->rd_ptr (),
                           message_block_p->length ());
      } while ((message_block_p = message_block_p->cont ()) != NULL);
      if (likely (Common_String_Tools::endswith (URL_string,
                                                 ACE_TEXT_ALWAYS_CHAR ("\r\n"))))
        URL_string.erase (URL_string.size () - 2);

      struct M3U_ExtInf_Element element_s;
      element_s.URL = URL_string;

      Test_I_Message* message_p = inherited::headFragment_;
      const Test_I_MessageDataContainer& data_container_2 = message_p->getR ();
      Test_I_IceCastClient_MessageData& data_2 =
        const_cast<Test_I_IceCastClient_MessageData&> (data_container_2.getR ());
      data_2.M3UPlaylist.ext_inf_elements.push_back (element_s);
