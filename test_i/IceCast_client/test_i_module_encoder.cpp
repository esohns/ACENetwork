...
Test_I_Encoder::Test_I_Encoder (ISTREAM_T* stream_in)
#else
...
bool
Test_I_Encoder::initialize (const struct Test_I_IceCastClient_ModuleHandlerConfiguration_2& configuration_in,
...
void
Test_I_Encoder::handleDataMessage (Test_I_Message*& message_inout,
...
void
Test_I_Encoder::handleSessionMessage (Test_I_SessionMessage_2*& message_inout,
                                      bool& passMessageDownstream_out)
{
  NETWORK_TRACE (ACE_TEXT ("Test_I_Encoder::handleSessionMessage"));
  ...
  switch (message_inout->type ())
  {
    case STREAM_SESSION_MESSAGE_BEGIN:
    {
      ...
      typename Test_I_IceCastClient_SessionData_2_t::DATA_T& session_data_r =
        const_cast<typename Test_I_IceCastClient_SessionData_2_t::DATA_T&> (inherited::sessionData_->getR ());
...
//audio:
      ...
      {
        ACE_DEBUG ((LM_ERROR,
                    ACE_TEXT ("%s: avcodec_find_encoder(%s) failed: \"%m\", aborting\n"),
                    inherited::mod_->name (),
                    ACE_TEXT (avcodec_get_name (audio_codec_id))));
        goto error;
      } // end IF
      ...
      {
        ACE_DEBUG ((LM_ERROR,
                    ACE_TEXT ("%s: avcodec_open2(%s) failed: \"%s\", aborting\n"),
                    inherited::mod_->name (),
                    ACE_TEXT (avcodec_get_name (audio_codec_id)),
                    ACE_TEXT (Common_Image_Tools::errorToString (result).c_str ())));
        goto error;
