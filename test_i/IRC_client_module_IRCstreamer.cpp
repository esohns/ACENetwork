...
IRC_Client_Module_IRCStreamer::IRC_Client_Module_IRCStreamer ()
 : inherited ()
{
  NETWORK_TRACE (ACE_TEXT ("IRC_Client_Module_IRCStreamer::IRC_Client_Module_IRCStreamer"));
...
void
IRC_Client_Module_IRCStreamer::handleDataMessage (IRC_Client_Message*& message_inout,
                                                  bool& passMessageDownstream_out)
{
  ...
  ACE_ASSERT (message_inout->space () >= IRC_CLIENT_IRC_FRAME_MAXSIZE);
...
// *PORTABILITY*
  ...
  // sanity check
  if (message_inout->space () < IRC_CLIENT_IRC_FRAME_BOUNDARY_SIZE)
  {
    ACE_DEBUG ((LM_ERROR,
                ACE_TEXT ("[%u]: out of buffer space (\"%s\", %u), aborting\n"),
                message_inout->getID (),
  ...
  *(message_inout->wr_ptr () + 1) = '\n';
  message_inout->wr_ptr (IRC_CLIENT_IRC_FRAME_BOUNDARY_SIZE);
