...
FTP_Module_Send_T<ACE_SYNCH_USE,
                  TimePolicyType,
                  ControlMessageType,
                  DataMessageType,
                  SessionMessageType,
                  ...
                  ConnectionStateType>::FTP_Module_Send_T (ISTREAM_T* stream_in)
#else
                  ConnectionStateType>::FTP_Module_Send_T (typename inherited::ISTREAM_T* stream_in)
#endif // ACE_WIN32 || ACE_WIN64
 : inherited (stream_in)
 , connection_ (NULL)
{
  NETWORK_TRACE (ACE_TEXT ("FTP_Module_Send_T::FTP_Module_Send_T"));
...
FTP_Module_Send_T<ACE_SYNCH_USE,
                  ...
                  SessionMessageType,
                  ConfigurationType,
                  ConnectionStateType>::~FTP_Module_Send_T ()
{
  NETWORK_TRACE (ACE_TEXT ("FTP_Module_Send_T::~FTP_Module_Send_T"));
...
{
  NETWORK_TRACE (ACE_TEXT ("FTP_Module_Send_T::handleDataMessage"));
...
continue_:
  ...
  {
    case FTP_STATE_INVALID:
    {
      if (!FTP_Tools::isSuccess (data_r.code))
        goto protocol_error;

      // --> greeting has been received; send MAIL command
      state_r.protocolState = FTP_STATE_GREETING_RECEIVED;
      ++state_r.protocolState;

      data_p->request.command = FTP_Codes::FTP_COMMAND_EHLO;
    ...
    case FTP_STATE_EHLO_SENT:
    {
      ...
      for (FTP_TextConstIterator_t iterator = data_r.text.begin ();
                     ...
                     ACE_TEXT_ALWAYS_CHAR ("LOGIN")) != authentication_mechanisms_a.end ())
      {
        ...
        data_p->request.command = FTP_Codes::FTP_COMMAND_AUTH;
        break;
      } // end IF
...
no_authentication_:
      ...
      state_r.protocolState = FTP_STATE_MAIL_SENT;

      data_p->request.command = FTP_Codes::FTP_COMMAND_MAIL;
    ...
    {
      if (data_r.code != FTP_Codes::FTP_CODE_SERVER_CHALLENGE)
        goto protocol_error;
    ...
    case FTP_STATE_MAIL_SENT:
    {
      ...
      data_p->request.command = FTP_Codes::FTP_COMMAND_RCPT;
    ...
    case FTP_STATE_RCPTS_SENT:
    {
      ...
      data_p->request.command = FTP_Codes::FTP_COMMAND_DATA;
      break;
    }
    case FTP_STATE_DATA_SENT:
    {
      if (data_r.code != FTP_Codes::FTP_CODE_START_MAIL_INPUT)
        goto protocol_error;

      // --> DATA has been sent; send message data
      ++state_r.protocolState;

      data_p->request.command = FTP_Codes::FTP_COMMAND_DATA_2;
    ...
    case FTP_STATE_DATA_2_SENT:
    {
      ...
      data_p->request.command = FTP_Codes::FTP_COMMAND_QUIT;
      break;
    }
    case FTP_STATE_QUIT_SENT:
    {
      ...
      state_r.protocolState = FTP_STATE_INVALID;
    ...
    default:
    {
      ACE_DEBUG ((LM_ERROR,
                  ...
                  inherited::mod_->name (),
                  ACE_TEXT (FTP_Tools::StateToString (state_r.protocolState).c_str ())));
      delete data_p; data_p = NULL;
      return;
    }
  } // end SWITCH
...
protocol_error:
  ACE_DEBUG ((LM_ERROR,
              ...
              inherited::mod_->name (),
              ACE_TEXT (FTP_Tools::StateToString (state_r.protocolState).c_str ()),
              data_r.code, ACE_TEXT (FTP_Tools::CodeToString (data_r.code).c_str ()),
              ACE_TEXT (FTP_Tools::dump (data_r).c_str ())));
...
FTP_Module_Send_T<ACE_SYNCH_USE,
                  ...
                  ConnectionStateType>::handleSessionMessage (SessionMessageType*& message_inout,
                                                              bool& passMessageDownstream_out)
{
  NETWORK_TRACE (ACE_TEXT ("FTP_Module_Send_T::handleSessionMessage"));
