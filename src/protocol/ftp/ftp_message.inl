...
// FTP_Message_T::FTP_Message_T(ACE_Allocator* messageAllocator_in)
...
FTP_Message_T<MessageType>::duplicate (void) const
{
  ...
  // create a new FTP_Message_T that contains unique copies of
...
allocate:
    try {
      ...
      ACE_NEW_MALLOC_NORETURN (message_p,
                               //static_cast<FTP_Message_T*>(message_block_allocator_->malloc(capacity())),
    ...
    if (inherited::message_block_allocator_)
    {
      ...
      if (allocator_p->block ())
        ACE_DEBUG ((LM_CRITICAL,
                    ACE_TEXT ("failed to allocate FTP_Message_T: \"%m\", aborting\n")));
    } // end IF
    else
      ACE_DEBUG ((LM_CRITICAL,
                  ACE_TEXT ("failed to allocate FTP_Message_T: \"%m\", aborting\n")));
  ...
  if (inherited::cont_)
  {
    ...
    {
      ACE_DEBUG ((LM_ERROR,
                  ACE_TEXT ("failed to FTP_Message_T::duplicate(): \"%m\", aborting\n")));
