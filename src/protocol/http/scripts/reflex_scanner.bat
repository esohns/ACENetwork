...
set FlexEXE="E:\lib\RE-flex\bin\win64\reflex.exe"
...
set LEX_FILE=reflex_scanner.l
if NOT exist "%LEX_FILE%" (
 echo invalid lex file ^(was: "%LEX_FILE%"^)^, exiting
 goto Failed
)
%FlexEXE% --noline --flex --bison-bridge --bison-locations %LEX_FILE% 2>scanner_report_scanner.txt
if %ERRORLEVEL% NEQ 0 (
...
@move /Y http_reflex_scanner.cpp .. >NUL
@move /Y http_reflex_scanner.h .. >NUL
if %ERRORLEVEL% NEQ 0 (
