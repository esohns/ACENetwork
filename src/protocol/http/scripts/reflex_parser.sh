...
SOURCE_FILE=${SCRIPTS_DIRECTORY}/reflex_parser.y
...
DOT_FILE=${SCRIPTS_DIRECTORY}/http_reflex_parser.dot
[ ! -f ${DOT_FILE} ] && echo "ERROR: file ${DOT_FILE} not found, aborting" && exit 1
IMAGE_FILE=http_reflex_parser.png
...
SOURCE_FILE=${SCRIPTS_DIRECTORY}/http_reflex_parser_patched.h
TARGET_FILE=${TARGET_DIRECTORY}/http_reflex_parser.h
[ ! -f ${SOURCE_FILE} ] && echo "ERROR: file ${SOURCE_FILE} not found, aborting" && exit 1
#cp -f ${SOURCE_FILE} ${TARGET_FILE}
#[ $? -ne 0 ] && echo "ERROR: failed to cp \"${SOURCE_FILE}\", aborting" && exit 1
#echo "copied \"$SOURCE_FILE\"..."
# clean up
SOURCE_FILE=http_reflex_parser.h
#rm -f ${SOURCE_FILE}
[ $? -ne 0 ] && echo "ERROR: failed to rm \"${SOURCE_FILE}\", aborting" && exit 1

FILES="http_reflex_parser.h http_reflex_parser.cpp"
#FILES="http_reflex_parser.cpp"
for FILE in $FILES
do
