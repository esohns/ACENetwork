...
%param                    { DHCP_ParserDriver* driver }
...
%type  <ival> message header cookie options

/* %printer                  { yyoutput << $$; } <*>; */
/* %printer                  { yyoutput << *$$; } <sval>
%printer                  { debug_stream () << $$; }  <ival> */
%printer                  { ACE_OS::fprintf (yyoutput, ACE_TEXT (" %s"), Net_Common_Tools::MACAddress2String ($$).c_str ()); } <aval>
...
//                                                                   ACE_TEXT (Net_Tools::IP2String (driver->record_->giaddr).c_str ())));
                                                       ACE_OS::memcpy (driver->record_->chaddr, $12, 16);
//                                                       ACE_DEBUG ((LM_DEBUG,
...
//                                                                   ACE_TEXT (driver->record_->file.c_str ())));
                    ...
                    | "end_of_fragment"              { $$ = $1;
                                                       yyclearin; };
options:            "key" "value" options            { ACE_ASSERT ($3);
                                                       driver->record_->options.insert (std::make_pair ($1, *$2)); };
                    | "end_of_fragment"              { $$ = $1;
                                                       YYACCEPT; };
%%
