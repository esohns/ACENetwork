...
template <typename MessageType>
SMTP_Message_T<MessageType>::SMTP_Message_T (const SMTP_Message_T<MessageType>& message_in)
...
// SMTP_Message_T::SMTP_Message_T(ACE_Allocator* messageAllocator_in)
...
template <typename MessageType>
SMTP_Code_t
SMTP_Message_T<MessageType>::command () const
{
  NETWORK_TRACE (ACE_TEXT ("SMTP_Message_T::command"));
...
template <typename MessageType>
void
SMTP_Message_T<MessageType>::dump_state () const
{
  NETWORK_TRACE (ACE_TEXT ("SMTP_Message_T::dump_state"));
...
template <typename MessageType>
ACE_Message_Block*
SMTP_Message_T<MessageType>::duplicate (void) const
{
  NETWORK_TRACE (ACE_TEXT ("SMTP_Message_T::duplicate"));

  OWN_TYPE_T* message_p = NULL;

  // create a new SMTP_Message_T that contains unique copies of
...
allocate:
    try {
      ...
      ACE_NEW_MALLOC_NORETURN (message_p,
                               //static_cast<SMTP_Message_T*>(message_block_allocator_->malloc(capacity())),
    ...
    if (inherited::message_block_allocator_)
    {
      ...
      if (allocator_p->block ())
        ACE_DEBUG ((LM_CRITICAL,
                    ACE_TEXT ("failed to allocate SMTP_Message_T: \"%m\", aborting\n")));
    } // end IF
    else
      ACE_DEBUG ((LM_CRITICAL,
                  ACE_TEXT ("failed to allocate SMTP_Message_T: \"%m\", aborting\n")));
  ...
  if (inherited::cont_)
  {
    ...
    {
      ACE_DEBUG ((LM_ERROR,
                  ACE_TEXT ("failed to SMTP_Message_T::duplicate(): \"%m\", aborting\n")));
