...
SMTP_SessionMessage_T<//AllocatorType,
                      SessionDataType,
                      UserDataType>::SMTP_SessionMessage_T (Stream_SessionId_t sessionId_in,
...
{
  NETWORK_TRACE (ACE_TEXT ("SMTP_SessionMessage_T::SMTP_SessionMessage_T"));
...
SMTP_SessionMessage_T<//AllocatorType,
                      SessionDataType,
                      UserDataType>::SMTP_SessionMessage_T (const SMTP_SessionMessage_T& message_in)
...
{
  NETWORK_TRACE (ACE_TEXT ("SMTP_SessionMessage_T::duplicate"));

  OWN_TYPE_T* message_p = NULL;

  // *NOTE*: create a new SMTP_SessionMessage_T that contains unique copies of
    ...
    if (inherited::message_block_allocator_)
    {
      ...
      if (allocator_p->block ())
        ACE_DEBUG ((LM_CRITICAL,
                    ACE_TEXT ("failed to allocate SMTP_SessionMessage_T: \"%m\", aborting\n")));
    } // end IF
    else
      ACE_DEBUG ((LM_CRITICAL,
                  ACE_TEXT ("failed to allocate SMTP_SessionMessage_T: \"%m\", aborting\n")));
