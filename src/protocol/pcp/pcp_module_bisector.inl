...
DHCP_Module_Bisector_T<LockType,
                      TaskSynchType,
                      TimePolicyType,
                      SessionMessageType,
                      ProtocolMessageType,
                      ...
                      StatisticContainerType>::DHCP_Module_Bisector_T ()
...
{
  NETWORK_TRACE (ACE_TEXT ("DHCP_Module_Bisector_T::DHCP_Module_Bisector_T"));

  if (DHCP_Bisector_lex_init (&context_))
    ACE_DEBUG ((LM_ERROR,
                ACE_TEXT ("failed to yylex_init(): \"%m\", continuing\n")));

  // trace ?
  DHCP_Bisector_set_debug ((DHCP_DEFAULT_LEX_TRACE ? 1 : 0),
                           context_);
}
                      ...
                      StatisticContainerType>::~DHCP_Module_Bisector_T ()
{
  ...
  if (context_)
    DHCP_Bisector_lex_destroy (context_);
...
{
  NETWORK_TRACE (ACE_TEXT ("DHCP_Module_Bisector_T::initialize"));
  ...
  if (isInitialized_)
  {
    ...
    if (bufferState_)
    {
      DHCP_Bisector__delete_buffer (bufferState_,
                                    context_);
      bufferState_ = NULL;
    } // end IF
    if (buffer_)
...
//                 statisticCollectionInterval_in));
  ...
  //  return false;
  //} // end IF
  DHCP_Bisector_set_debug ((configuration_in.traceScanning ? 1 : 0),
                           context_);
...
{
  NETWORK_TRACE (ACE_TEXT ("DHCP_Module_Bisector_T::handleDataMessage"));
  ...
  //         --> more sanity check(s)
  //if (buffer_->total_length () < DHCP_FRAME_BOUNDARY_SIZE)
...
//   while (messageLength_ = scanner_.yylex ())
  do
  {
    scanned_chunk = DHCP_Bisector_lex (context_);
//    scanned_chunk = IRC_Bisectlex (context_);
    switch (scanned_chunk)
    {
      ...
      case -1:
      {
        ...
        {
          scanned_bytes = 1;
          //messageLength_ += DHCP_FRAME_BOUNDARY_SIZE;
        } // end IF
        else
        {
          //scanned_bytes += DHCP_FRAME_BOUNDARY_SIZE;
          messageLength_ += scanned_bytes;
        } // end IF
...
DHCP_Module_Bisector_T<LockType,
                      ...
                      StatisticContainerType>::handleSessionMessage (SessionMessageType*& message_inout,
                                                                     bool& passMessageDownstream_out)
{
  NETWORK_TRACE (ACE_TEXT ("DHCP_Module_Bisector_T::handleSessionMessage"));
...
{
  NETWORK_TRACE (ACE_TEXT ("DHCP_Module_Bisector_T::collect"));
...
{
  NETWORK_TRACE (ACE_TEXT ("DHCP_Module_Bisector_T::report"));
...
{
  NETWORK_TRACE (ACE_TEXT ("DHCP_Module_Bisector_T::putStatisticMessage"));
...
DHCP_Module_Bisector_T<LockType,
                      ...
                      StatisticContainerType>::scan_begin (char* data_in,
                                                           size_t length_in)
{
  NETWORK_TRACE (ACE_TEXT ("DHCP_Module_Bisector_T::scan_begin"));
...
//  // *IMPORTANT NOTE*: length_in IS already adjusted for two trailing \0's
  ...
  //                                                         context_);
  bufferState_ =
      DHCP_Bisector__scan_bytes (data_in,
                                 length_in,
                                 context_);
  if (!bufferState_)
  {
  ...
  DHCP_Bisector__switch_to_buffer (bufferState_,
                                   context_);
...
{
  NETWORK_TRACE (ACE_TEXT ("DHCP_Module_Bisector_T::scan_end"));
