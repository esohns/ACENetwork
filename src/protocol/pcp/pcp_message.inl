...
template <//typename AllocatorConfigurationType,
          typename MessageType>
PCP_Message_T<//AllocatorConfigurationType,
               MessageType>::PCP_Message_T (const PCP_Message_T<//AllocatorConfigurationType,
...
// PCP_Message_T::PCP_Message_T(ACE_Allocator* messageAllocator_in)
...
{
  NETWORK_TRACE (ACE_TEXT ("PCP_Message_T::command"));

  // sanity check(s)
  if (!inherited::isInitialized_)
    return PCP_Codes::PCP_OPCODE_INVALID;

  return static_cast<PCP_Opcode_t> (inherited::data_.opcode & 0x7f);
}
...
{
  NETWORK_TRACE (ACE_TEXT ("PCP_Message_T::dump_state"));
  ...
  ACE_DEBUG ((LM_INFO,
              ACE_TEXT ("\n%s"),
              ACE_TEXT (PCP_Tools::dump (inherited::data_).c_str ())));
}
...
{
  NETWORK_TRACE (ACE_TEXT ("PCP_Message_T::duplicate"));

  OWN_TYPE_T* message_p = NULL;

  // create a new PCP_Message_T that contains unique copies of
...
allocate:
    try {
      ...
      ACE_NEW_MALLOC_NORETURN (message_p,
                               //static_cast<PCP_Message_T*>(message_block_allocator_->malloc(capacity())),
    ...
    if (inherited::message_block_allocator_)
    {
      ...
      if (allocator_p->block ())
        ACE_DEBUG ((LM_CRITICAL,
                    ACE_TEXT ("failed to allocate PCP_Message_T: \"%m\", aborting\n")));
    } // end IF
    else
      ACE_DEBUG ((LM_CRITICAL,
                  ACE_TEXT ("failed to allocate PCP_Message_T: \"%m\", aborting\n")));
  ...
  if (inherited::cont_)
  {
    ...
    {
      ACE_DEBUG ((LM_ERROR,
                  ACE_TEXT ("failed to PCP_Message_T::duplicate(): \"%m\", aborting\n")));
