...
#define FREE(x) HeapFree(GetProcessHeap(), 0, (x))
  std::string interface_identifier =
    Net_Common_Tools::interfaceToString (interfaceIdentifier_in);
...
#endif // COMMON_OS_WIN32_TARGET_PLATFORM(0x0600)
#if COMMON_OS_WIN32_TARGET_PLATFORM(0x0600) // _WIN32_WINNT_VISTA
      if (unlikely (InlineIsEqualGUID (result, GUID_NULL)))
#else
...
#else
      Net_Common_Tools::indexToInterface_2 (route_s.dwForwardIfIndex);
#endif // COMMON_OS_WIN32_TARGET_PLATFORM(0x0600)
#if COMMON_OS_WIN32_TARGET_PLATFORM(0x0600) // _WIN32_WINNT_VISTA
    if (unlikely (InlineIsEqualGUID (result, GUID_NULL)))
#else
...
#else
        interface_identifiers_a.push_back (Net_Common_Tools::indexToInterface_2 (Net_Common_Tools::interfaceToIndex (interface_list_p->InterfaceInfo[i].InterfaceGuid)));
#endif // COMMON_OS_WIN32_TARGET_PLATFORM(0x0600)
...
bool
Net_Common_Tools::connectDHClient (const ACE_INET_Addr& address_in,
                                   dhcpctl_handle authenticator_in,
                                   Net_DHClientCallback_t callback_in,
                                   void* userData_in,
                                   dhcpctl_handle& connection_out)
{
  NETWORK_TRACE (ACE_TEXT ("Net_Common_Tools::connectDHClient"));
  ...
  if (likely (callback_in))
  {
    ...
    if (unlikely (status_i != ISC_R_SUCCESS))
    {
      ACE_DEBUG ((LM_ERROR,
                  ...
                  callback_in,
                  ACE_TEXT (isc_result_totext (status_i))));
      Net_Common_Tools::disconnectDHClient (connection_out);
...
void
Net_Common_Tools::disconnectDHClient (dhcpctl_handle& connection_inout)
{
  NETWORK_TRACE (ACE_TEXT ("Net_Common_Tools::disconnectDHClient"));
...
bool
Net_Common_Tools::getInterfaceState (dhcpctl_handle connection_in,
                                     const std::string& interfaceIdentifier_in,
                                     Net_DHClientCallback_t callback_in,
                                     void* userData_in)
{
  NETWORK_TRACE (ACE_TEXT ("Net_Common_Tools::getInterfaceState"));
...
bool
Net_Common_Tools::hasState (dhcpctl_handle handle_in,
                            const std::string& state_in)
{
  NETWORK_TRACE (ACE_TEXT ("Net_Common_Tools::hasState"));
...
bool
Net_Common_Tools::relinquishLease (dhcpctl_handle connection_in,
                                   const std::string& interfaceIdentifier_in)
{
  NETWORK_TRACE (ACE_TEXT ("Net_Common_Tools::relinquishLease"));
