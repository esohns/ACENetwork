...
const char stream_name_string_[] = ACE_TEXT_ALWAYS_CHAR ("UPnPClientStream");
                     ...
                     unsigned int& numberOfDispatchThreads_out)
{
  ...
  // initialize results
  //requestBroadcastReplies_out = UPnP_Client_DEFAULT_FLAGS_BROADCAST;
  ...
  //sendRequestOnOffer_out = TEST_U_DEFAULT_UPnP_Client_SEND_REQUEST_ON_OFFER;
  useReactor_out =
      (COMMON_EVENT_DEFAULT_DISPATCH == COMMON_EVENT_DISPATCH_REACTOR);
...
do_work (//bool requestBroadcastReplies_in,
         ...
         struct UPnP_Client_Configuration& configuration_in,
#if defined (GUI_SUPPORT)
         struct UPnP_Client_UI_CBData& CBData_in,
#endif // GUI_SUPPORT
         ...
         Test_U_SignalHandler& signalHandler_in)
{
  ...
  UPnP_Client_MessageAllocator_t message_allocator (TEST_U_MAX_MESSAGES, // maximum #buffers
  ...
  if (useReactor_in)
    ...
    connection_configuration_outbound_multicast.socketConfiguration.connect =
      true;
  ...
  connection_configuration_outbound_multicast.messageAllocator = &message_allocator;
  connection_configuration_outbound_multicast.streamConfiguration =
    &configuration_in.streamConfiguration;
  configuration_in.connectionConfigurations.insert (std::make_pair (ACE_TEXT_ALWAYS_CHAR ("Out"),
                                                                    &connection_configuration_outbound_multicast));
  ...
  connection_configuration_inbound_unicast =
    connection_configuration_outbound_multicast;
  connection_configuration_inbound_unicast.delayRead = false;
...
//  struct UPnP_Client_StreamConfiguration stream_configuration;
  ...
  connection_configuration_inbound_multicast.socketConfiguration.reuseAddress =
    false;
  result =
    connection_configuration_inbound_multicast.socketConfiguration.listenAddress.set (static_cast<u_short> (SSDP_DEFAULT_SERVER_PORT),
                                                                                      ACE_TEXT_ALWAYS_CHAR (SSDP_ADDRESS_IPV4_MULTICAST),
                                                                                      1, // encode
                                                                                      AF_INET);
  if (result == -1)
  ...
  connection_configuration_inbound_multicast.socketConfiguration.peerAddress.set (static_cast<u_short> (0),
                                                                                  static_cast<ACE_UINT32> (INADDR_ANY),
                                                                                  1, // encode
                                                                                  0);
  ...
  //UPnP_Client_ISession_t* isession_p = NULL;
  if (UIDefinitionFileName_in.empty ())
  {
    ...
    iterator =
      configuration_in.connectionConfigurations.find (ACE_TEXT_ALWAYS_CHAR ("Out"));
    ACE_ASSERT (iterator != configuration_in.connectionConfigurations.end ());
    //isession_p->initialize (*static_cast<UPnP_Client_ConnectionConfiguration*> ((*iterator).second),
    //                        iconnection_p);

    // step2: send UPnP/SSDP request
    ...
    //struct UPnP_Client_SessionState& state_r =
    //    const_cast<struct UPnP_Client_SessionState&> (isession_p->getR ());
           ...
           ACE_TCHAR* argv_in[])
{
  ...
  // step1a set defaults
  //bool request_broadcast_replies = UPnP_Client_DEFAULT_FLAGS_BROADCAST;
  ...
  //    TEST_U_DEFAULT_UPnP_Client_SEND_REQUEST_ON_OFFER;
  bool use_reactor =
      (COMMON_EVENT_DEFAULT_DISPATCH == COMMON_EVENT_DISPATCH_REACTOR);
...
//  lock_2 = &state_r.subscribersLock;
  ...
  struct UPnP_Client_Configuration configuration_s;
#if defined (GUI_SUPPORT)
  // step1h: initialize GLIB / G(D|T)K[+] / GNOME
  struct UPnP_Client_UI_CBData ui_cb_data;
