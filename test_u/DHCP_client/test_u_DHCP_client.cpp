                     ...
                     unsigned int& numberOfDispatchThreads_out)
{
  ...
  port_out = DHCP_DEFAULT_SERVER_PORT;
    ...
    switch (option)
    {
      ...
      case 'u':
      {
        ...
        std::string regex_string =
          ACE_TEXT_ALWAYS_CHAR ("^(?:DHCP://)?([[:alnum:]-.]+)(?:\\:([[:digit:]]{1,5}))?(.+)?$");
...
//            ACE_TEXT_ALWAYS_CHAR ("^(\\/.+(?=\\/))*\\/(.+?)(\\.(html|htm))?$");
        regex_string =
            ACE_TEXT_ALWAYS_CHAR ("^(?:DHCP://)?((.+\\.)+([^\\/]+))(\\/.+(?=\\/))*\\/(.+?)(\\.(html|htm))?$");
...
allocate:
  ...
  message_data_p->DHCPRecord->method = DHCP_Codes::DHCP_METHOD_GET;
  message_data_p->DHCPRecord->URI = URL_in;
  message_data_p->DHCPRecord->version = DHCP_Codes::DHCP_VERSION_1_1;
           ...
           ACE_TCHAR* argv_in[])
{
  ...
  unsigned short port = DHCP_DEFAULT_SERVER_PORT;
